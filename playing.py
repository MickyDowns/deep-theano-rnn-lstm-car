...
TURN_NUM_SENSOR = 10 # front five sonar color / distance readings
...
AVOID_NUM_SENSOR = 16 # seven sonar distance readings, turn action, current speed
...
HUNT_AVOID = 0
HUNT_ACQUIRE = 1
HUNT_NUM_SENSOR = 16 # 7 sonar sensors, distance, heading.
...
def play(turn_model, turn_model_30, turn_model_50, turn_model_70, avoid_model, acquire_model,
    ...
    turn_state, avoid_state, acquire_state, hunt_state, drone_state, reward, cur_speed = \
        game_state.frame_step(START_DRONE_ID, START_TURN_ACTION, START_SPEED_ACTION, START_PACK_ACTION, START_SPEED, START_DISTANCE, 1)
    ...
    while True:
        ...
        for drone_id in range(NUM_DRONES): # NUM_DRONES = 1, unless you're in PACK mode
            ...
            # choose action.
            avoid_action = START_SPEED_ACTION
            speed_action = START_SPEED
            pack_action = START_PACK_ACTION
                ...
                if cur_mode == AVOID:
                    ...
                    if cur_mode == ACQUIRE:
                        ...
                        if cur_mode == HUNT:
                            ...
                            if cur_mode == PACK and replay_frame_ctr % PACK_EVAL_FRAMES == 0 and drone_id == 0:
                                # get 1 pack action for each set of drones on first drone
                                last_pack_action = pack_action
            ...
            if cur_mode in [TURN, AVOID, HUNT, PACK]:
                new_turn_state = state_frames(new_turn_state, np.array([turn_states[drone_id]]),
                                              TURN_NUM_SENSOR,TURN_STATE_FRAMES)
        
            if cur_mode in [AVOID, HUNT, PACK]:
                new_avoid_state = state_frames(new_avoid_state, np.array([avoid_states[drone_id]]),
            ...
            if cur_mode in [HUNT, PACK]:
                new_hunt_state = state_frames(new_hunt_state, np.array([hunt_states[drone_id]]),
        ...
        # Tell us something.
        if crash_frame_ctr % 1000 == 0:
            print("Current distance: %d frames." % crash_frame_ctr)
...
if __name__ == "__main__":
    ...
    if cur_mode == PACK:
        ...
        if cur_mode == PACK and use_existing_model == False:
            ...
            saved_model = 'models/pack/pack-720-180-100-50000-50000.h5'
            pack_model = pack_net(PACK_NUM_INPUT, nn_param,
                                  PACK_NUM_OUTPUT, saved_model)
