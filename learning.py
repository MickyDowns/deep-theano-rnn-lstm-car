...
def train_net(turn_model, turn_model_30, turn_model_50, turn_model_70, avoid_model, acquire_model,
    ...
    train_frames = 500000  # number of flips for training
    ...
    while total_frame_ctr < train_frames:
        ...
        for drone_id in range(NUM_DRONES): # NUM_DRONES = 1, unless you're in PACK mode
                ...
                if cur_mode == TURN:
                    ...
                    if cur_mode == AVOID:
                        ...
                        if cur_mode == ACQUIRE:
                            ...
                            if cur_mode == HUNT:
                                hunt_action, turn_action, speed_action = set_hunt_action(True, cur_speeds[drone_id], turn_action,speed_action, acquire_action, np.array([hunt_states[drone_id,]]))
                            else:
            ...
            if new_reward == -500 or new_reward == -1000:
                ...
                #try:
                #    print("Max: %d at %d\t eps: %f\t dist: %d\t mode: %d\t cum rwd: %d\t fps: %d" % #(max_crash_frame_ctr, total_frame_ctr, epsilon, crash_frame_ctr, cur_mode, cum_rwd, int(fps)))
                #    break
