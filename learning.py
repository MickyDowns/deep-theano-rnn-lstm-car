...
def train_net(model, params):
    ...
    _, state = game_state.frame_step((2)) # frame_step returns reward, state
    state = state_frames(state, np.array([[0, 0, 0]])) # zeroing distance readings
    ...
    while t < train_frames:
        ...
        if random.random() < epsilon or t < observe:
            ...
            # model was passed to this function. here, it's calling model w/ current state
        ...
        if t > observe:
            ...
            minibatch = random.sample(replay, batchSize) # why random sample?
        ...
        # Save early model, then every 20,000 frames
        if t % 50000 == 0:
            save_init = False
