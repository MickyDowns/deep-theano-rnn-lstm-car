...
SPEED_NUM_SENSOR = 7 # front five sonar distance readings, turn action, current speed
SPEED_NUM_OUTPUT = 4 # do nothing, 30, 50, 70
SPEED_NUM_FRAME = 3
...
def train_net(turn_model, turn_model_1, turn_model_2, turn_model_3,
              speed_model, acquire_model, params):
    ...
    car_distance = 0
    t = 0
    cum_rwd = cum_rwd_turn = cum_rwd_speed = cum_rwd_acquire = \
    cum_rwd_hunt = cum_speed = 0
    data_collect = []
    ...
    while t < train_frames:
        ...
        if random.random() < epsilon or t < observe: # epsilon degrades over flips...
            ...
            if cur_mode == SPEED:
                ...
                # kludgie, but ensures speed is using curren turn prediction
        ...
        if new_reward == -500 or new_reward == -1000:
            ...
            print("Max: %d at %d\t eps: %f\t dist: %d\t rwd: %d\t turn: %d\t acq: %d\t spd: %d\t avg spd: %f\t fps: %d" %
                  (max_car_distance, t, epsilon, car_distance, cum_rwd, cum_rwd_turn,
                   cum_rwd_acquire, cum_rwd_speed, cum_speed / car_distance, int(fps)))
        ...
        if t % 10000 == 0 & car_distance != 0:
            ...
            print("t: %d\t eps: %f\t dist: %d\t rwd: %d\t turn: %d\t acq: %d\t spd: %d\t avg spd: %f\t" %
                  (t, epsilon, car_distance, cum_rwd, cum_rwd_turn, cum_rwd_acquie,
                   cum_rwd_speed, cum_speed / car_distance))
